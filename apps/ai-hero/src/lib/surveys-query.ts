	...
	try {
		...
		const result = await db.execute(sql`
			SELECT
				...
				u.emailVerified AS user_emailVerified,
				u.image AS user_image,
				u.createdAt AS user_createdAt,
				q.id AS question_resource_id,
				q.type AS question_resource_type,
				q.createdById AS question_resource_createdById,
				q.currentVersionId AS question_resource_currentVersionId,
				q.fields AS question_resource_fields,
				q.slug AS question_resource_slug,
				q.createdAt AS question_resource_createdAt,
				q.updatedAt AS question_resource_updatedAt,
				q.deletedAt AS question_resource_deletedAt,
				q.organizationId AS question_resource_organizationId,
				q.createdByOrganizationMembershipId AS question_resource_createdByOrganizationMembershipId
			FROM ${questionResponse} qr
			LEFT JOIN ${users} u ON qr.userId = u.id
			LEFT JOIN ${contentResource} q ON qr.questionId = q.id
			WHERE qr.surveyId = ${surveyId}
			ORDER BY qr.createdAt DESC
		`)

		const responses = result.rows.flatMap((row: any) => {
			if (!row.question_resource_id) return []
			...
			return [
				{
					...
					userId: row.userId,
					emailListSubscriberId: row.emailListSubscriberId,
					fields: row.fields,
					...
					user: row.user_id
						...
						: null,
					question: {
						id: row.question_resource_id,
						type: row.question_resource_type,
						createdById: row.question_resource_createdById,
						currentVersionId: row.question_resource_currentVersionId,
						fields: row.question_resource_fields,
						slug: row.question_resource_slug,
						createdAt: row.question_resource_createdAt,
						updatedAt: row.question_resource_updatedAt,
						deletedAt: row.question_resource_deletedAt,
						resources: [],
						organizationId: row.question_resource_organizationId,
						createdByOrganizationMembershipId:
							row.question_resource_createdByOrganizationMembershipId,
					},
				},
			]
		})
		...
		if (!parsedResponses.success) {
			await log.error('survey.responses.parse.failed', {
				surveyId,
				error: parsedResponses.error.format(),
			})
			return null
		}
		return parsedResponses.data
	} catch (error) {
		await log.error('survey.responses.load.failed', {
			surveyId,

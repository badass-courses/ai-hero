...
import { getAllUserEntitlements } from './entitlements-query'
...
export async function getCohort(cohortIdOrSlug: string) {
	...
	if (!parsedCohort.success) {
		void log.error('cohort.parse.error', {
			cohortIdOrSlug,
			cohortId: cohortData?.id,
			error: parsedCohort.error.message,
		})
		return null
	}
...
export async function getOpenCohort() {
	...
	const cohortsData = await db.query.contentResource.findMany({
	...
	const cohorts = z.array(CohortSchema).safeParse(cohortsData)
	if (!cohorts.success) return null
	...
	for (const cohort of cohorts.data) {
		if (cohort.fields.visibility !== 'public') continue

		const product = cohort.resourceProducts?.[0]?.product
		if (!product) continue

		const { openEnrollment, closeEnrollment } = product.fields ?? {}
		if (!openEnrollment) continue

		const isOpen =
			new Date(openEnrollment) < now &&
			(!closeEnrollment || new Date(closeEnrollment) > now)
		...
		const hasPurchased = userPurchases?.some(
			(p: PurchaseSchema) =>
				...
				(p.status === 'Valid' || p.status === 'Restricted'),
		)
...
function getWorkshopResourcesFromCohort(cohort: Cohort): Workshop[] {
	return (cohort.resources ?? [])
		.map((resourceItem) => resourceItem.resource)
		.filter((resource): resource is Workshop => resource?.type === 'workshop')
}
...
export async function loadCohortPageData(
	...
	const workshops = getWorkshopResourcesFromCohort(cohort)
	const workshopIds = workshops.map((workshop) => workshop.id)
	let hasAccessToCohort = false

	if (ability?.can('update', 'Content')) {
		hasAccessToCohort = true
	} else if (user?.id && workshopIds.length > 0) {
		...
		if (cohortEntitlementType) {
			...
			const hasEntitlementAccess = activeEntitlements.some((entitlement) => {
				if (entitlement.entitlementType !== cohortEntitlementType.id)
					return false
				...
				return contentIds.some((contentId) => workshopIds.includes(contentId))
			})

			const cohortAbility = createAppAbility(
				hasEntitlementAccess
					? [
							{
								action: 'read',
								subject: 'Content',
								conditions: { id: { $eq: cohort.id } },
							},
						]
					: [],
			)

			hasAccessToCohort = cohortAbility.can(
				'read',
				subject('Content', { id: cohort.id }),
			)
		}
	}
...
export async function getAllWorkshopsInCohort(cohortId: string) {
	try {
		...
		return results.map((r) => {
			...
			if (!parsed.success) {
				void log.error('cohort.workshop.parse.error', {
					cohortId,
					workshopId: r.ContentResource?.id,
		...
		void log.error('cohort.workshop.query.error', {
			cohortId,
...
export async function syncDiscordRoles(organizationId: string, user: User) {
	...
	try {
		...
		void log.error('cohort.discord.sync.error', {
			organizationId,
			userId: user.id,
			error: error instanceof Error ? error.message : String(error),
		})

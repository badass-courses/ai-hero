...
import { createCompletedValuePathIntentScan } from './value-path-intent-scan'
// Independent array oracle retained from the pre-streaming implementation.
...
type CompletedValuePathIntentScanDiagnostics = {
	scanned: number
	eligible: number
	frontierSize: number
...
function referenceScan(args: CompletedValuePathIntentScanArgs): {
	intents: SideEffectIntent[]
...
function selectCompletedValuePathIntentFrontier(
	args: CompletedValuePathIntentScanArgs,
): SideEffectIntent[] {
	return referenceScan(args).intents
}

function sortValuePathIntentsByCreatedAt(intents: SideEffectIntent[]) {
	return [...intents].sort(
		(a, b) =>
...
function hasValidCompletedAt(intent: SideEffectIntent) {
...
it('matches array frontier and every diagnostic over 100001 mixed historical rows', () => {
	const intents: SideEffectIntent[] = Array.from(
		{ length: 100_001 },
		(_, index) => ({
			id: `intent-${index}`,
			contactId: `contact-${index % 1001}`,
			nextActionId: 'action',
			provider: 'kit',
			...
			metadata: {
				...
				emailResourceId: `ai-hero-skills-workflow.email-${index % 8}`,
				kitSequenceId: 'sequence',
				...(index % 5 ? { completedAt: '2026-08-01T00:00:00.000Z' } : {}),
			},
		}),
	)
	for (const scope of [undefined, [], ['contact-0', 'contact-2']]) {
		const args = {
			limit: 17,
			contactIds: scope,
		...
		for (let offset = 0; offset < intents.length; offset += 337)
			scan.addPage(intents.slice(offset, offset + 337))
		expect(scan.finish()).toEqual(referenceScan({ ...args, intents }))
	}
}, 30_000)

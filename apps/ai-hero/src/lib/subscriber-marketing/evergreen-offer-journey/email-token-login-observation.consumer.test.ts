...
const types = [
	EMAIL_TOKEN_LOGIN_OBSERVED,
	OFFER_CLAIM_OBSERVED,
	'evergreen.delivery-mapping.recorded',
]
async function fixture(type: string, stored = true) {
	...
	const event = {
		...
		summary: 'Internal configuration record.',
	}
	...
	return {
		...
		initial,
		event,
		lookup: new InMemoryOperatorLookupRepository(repository),
	}
}
afterEach(() => {
	vi.restoreAllMocks()
...
describe('exact internal observation preview exceptions', () => {
	it.each(
		types.flatMap((type) =>
			[true, false].flatMap((stored) =>
				[true, false].map((explicit) => ({ type, stored, explicit })),
			),
		),
	)(
		'$type stored=$stored explicit=$explicit',
		async ({ type, stored, explicit }) => {
			const f = await fixture(type, stored),
				before = structuredClone([...f.repository.states.values()])
			...
			const result = await previewSubscriberMarketingReplay({
				repository: f.lookup,
				contactId: f.initial.contact.id,
				...(explicit ? { eventId: f.event.id } : {}),
			})
			expect(result.mode).toBe('non-behavioral-replay-preview')
			if (result.mode !== 'non-behavioral-replay-preview')
				throw new Error('Expected exact exception')
			expect(result.preview.contactEvent.id).toBe(f.event.id)
			expect(result.preview.state).toBe(
	...
	it.each(types)(
		'retains Contact and ProviderIdentity checks for %s',
		async (type) => {
			const f = await fixture(type)
			f.repository.contactEvents.set(f.event.id, {
				...f.event,
				contactId: 'other',
			})
			await expect(
			...
			).rejects.toThrow('No replayable event')
			f.repository.contactEvents.set(f.event.id, {
				...f.event,
				providerIdentityId: 'other',
			})
			await expect(
				previewSubscriberMarketingReplay({
					repository: f.lookup,
	...
	it.each([
		'auth.other',
		'evergreen.offer_claim_observed.v2',
		'ordinary.event',
	])('does not introduce a wildcard for %s', async (type) => {
		const f = await fixture(type)
		const result = await previewSubscriberMarketingReplay({
			repository: f.lookup,
			contactId: f.initial.contact.id,
		})
		expect(result.mode).toBe('replay-preview')
		expect(result.preview.classification).not.toBeNull()
	})
})

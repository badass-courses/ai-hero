...
- Native MySQL boundary (no `provider` field): `aih_contact_maintenance_reader` for inspect/dry-run/verify, `aih_contact_maintenance_writer` for apply, with matching SQL `CURRENT_USER()` semantics. These are not PlanetScale usernames.
- PlanetScale boundary: add `provider: "planetscale"`, `organization`, `branch`, and `operatorRole: "reader" | "writer"`. `user` is the actual provider-generated username. Supply a separate private `--provider-receipt-fd` and an independently approved `--provider-receipt-sha256`. Do not use proxy `CURRENT_USER()` as external credential identity.

The private receipt contains version1/provider/purpose, operatorRole/target/approvalRef, organization/database/branch/credentialName, readbackAt/validUntil, and the original `pscale password create --format json` object as `creation` plus its password readback object as `readback`. Actual JSON uses `id`, `username`, `access_host_url`, `database_branch.name`, `role`, `plain_text`, `created_at`, `expires_at`, `deleted_at`, `ttl_seconds`, `replica`; it is not the human/CSV password display. Source: [PlanetScale CLI cf7e2942](https://github.com/planetscale/cli/tree/cf7e2942e103bf0d9ce4c2e489aeebcbe8a6fe1e), `internal/planetscale/passwords.go` and the password command's `MarshalJSON`.

The approval packet must pin the exact receipt bytes independently of the credential file. Obtain both through the already-authorized operator provider path; this command never creates passwords, queries provider APIs or treats a caller's hash/purpose strings as verified provider authorization. It checks the supplied artifact's consistency and pin, not its cryptographic origin or current revocation state. Records must agree on ID, name, username, host, branch, creation identity and secret. Organization/database/target/mode/approval reference must match the receipt scope. Inspect/dry-run/verify need provider `reader`; apply needs `readwriter` because it also reads. Provider `writer` alone and `admin` are rejected. Replica/deleted/expired records fail closed. The approved receipt validity window and any provider expiry must cover the operation; a no-TTL provider password still needs that bounded receipt validity. Password labels/purpose are separate from generated usernames. Private cursors also bind provider organization/branch/password ID/role. The pinned receipt also bounds the narrow provider snapshot support described below.

Account provisioning and privileges need their own approval. Native column-scoped UPDATE is not asserted to describe PlanetScale role permissions; the maintenance service itself still writes only the two projection columns.
...
A future approved invocation has this shape from `apps/ai-hero` (fd 3 must already be supplied). Invoke Node directly for inherited descriptors: package-manager child processes may close fd 3 and above. The offline commands above need no extra descriptors.

```sh
node --import tsx src/scripts/contact-integrity-maintenance.ts \
...
Whole verification supports native MySQL 8.4 Community/InnoDB and the exercised PlanetScale 8.4.11 backend with its empty version comment. The provider path additionally requires this process's immutable validated receipt, bound to verify mode, the actual `DATABASE()` and an expiry covering the remaining operation. Missing, copied, expired or mismatched evidence and unknown provider versions fail with `unsupported-snapshot`. A version string alone does not enable the provider path.

Isolated schema-only development-branch experiments exercised the installed mysql2/Drizzle owned transactions, stale/duplicate/raw-email writer blocking until commit, rollback, server lock timeout, read-only write refusal and stable snapshot reads across concurrent commits. The actual bounded verifier retained its two-row snapshot while a concurrent third row and stale projection appeared only in the later observation. These are development-branch observations, not production permissions, production latency guarantees or proof of behavior during every online schema-migration phase. No destructive native harness is used against the provider.

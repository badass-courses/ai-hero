...
describe('linking a contact to its first Kit subscriber', () => {
	function linker(existing: { owner?: string; contactKitId?: string } = {}) {
		const requestSync = vi.fn(async () => 'requested' as const)
		const createProviderIdentity = vi.fn(async () => undefined)
		const link = createKitSubscriberLinker({
			repository: {
				findProviderIdentity: async () =>
					existing.owner ? { contactId: existing.owner } : undefined,
		...
		return { link, requestSync, createProviderIdentity }
	}

...
describe('course resume navigation', () => {
	...
	it('continues from a final lesson to the next workshop in the product', () => {
		const firstWorkshop = resource('workshop-1', 'workshop', [
			resource('lesson-1', 'lesson'),
		])
		const firstLessonInNextWorkshop = resource('lesson-2', 'lesson')
		const nextWorkshop = resource('workshop-2', 'workshop', [
			resource('section-2', 'section', [firstLessonInNextWorkshop]),
		])
		const cohort = resource('cohort-1', 'cohort', [
			firstWorkshop,
			nextWorkshop,
		...
		expect(getNextWorkshopLesson([cohort], firstWorkshop.id)).toEqual({
			lesson: firstLessonInNextWorkshop,
			workshop: nextWorkshop,
			href: '/workshops/workshop-2/lesson-2',
		})
	})

	it('ends only after the product final workshop', () => {
		const workshop = resource('workshop-1', 'workshop', [
			resource('lesson-1', 'lesson'),
		])

		expect(getNextWorkshopLesson([workshop], workshop.id)).toBeNull()
	})
})

...
ORDER BY priority ASC;`
	...
	if (!parsedProducts.success) {
		void log.error('workshop.product.parse.error', {
			scope: 'all',
			workshopIdOrSlug,
			error: parsedProducts.error.message,
		})
		return []
	}
...
LIMIT 1;`
	...
	if (!parsedProduct.success) {
		void log.error('workshop.product.parse.error', {
			scope: 'single',
			workshopIdOrSlug,
			error: parsedProduct.error.message,
		})
		return null
	}
...
export async function getMinimalWorkshop(moduleSlugOrId: string) {
	return measureIfSlow({
		event: 'perf.workshop.minimal-fetch.slow',
		spanName: 'workshop.minimal.fetch',
		thresholdMs: 100,
		data: { moduleSlugOrId },
		operation: async () => {
			...
			if (!workshop) {
				await log.error('workshop.query.not-found', {
					moduleSlugOrId,
				})
				return null
			}
...
export async function getWorkshop(moduleSlugOrId: string) {
	return measureIfSlow({
		event: 'perf.workshop.fetch.slow',
		spanName: 'workshop.fetch',
		thresholdMs: 150,
		data: { moduleSlugOrId },
		operation: async () => {
			...
			if (!parsedWorkshop.success) {
				void log.error('workshop.parse.error', {
					...
					error: parsedWorkshop.error.message,
				})
				return null
			}
...
export async function updateWorkshop(input: Partial<Workshop>) {
	...
	if (input.fields?.slug !== currentWorkshop.fields.slug) {
		...
		await log.info('workshop.update.slug.manual', {
			workshopId: input.id,
	...
	try {
		...
		await log.info('workshop.update.typesense.success', {
			workshopId: currentWorkshop.id,
			action: 'save',
			userId: user.id,
		})
	} catch (error) {
		await log.error('workshop.update.typesense.failed', {
			workshopId: currentWorkshop.id,
			action: 'save',
			userId: user.id,
	...
	revalidatePath(`/workshops/${workshopSlug}/for-teams`)
...
export async function getWorkshopsForLesson(lessonId: string) {
	...
	if (!parsedWorkshops.success) {
		await log.error('workshop.lesson.parse.error', {
			lessonId,
			error: parsedWorkshops.error.message,
		})
		return []

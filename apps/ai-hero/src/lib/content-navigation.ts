...
const completionTrackedResourceTypes = new Set(['lesson', 'exercise', 'post'])

export function isCompletionTrackedResource(
	resource: ContentResource | null | undefined,
): resource is ContentResource {
	return Boolean(resource && completionTrackedResourceTypes.has(resource.type))
}

export function getModuleCompletionState({
	navigation,
	completedLessons,
	resourceIdToMarkComplete,
}: {
	navigation: ResourceNavigation | null
	completedLessons?:
		| Pick<ResourceProgress, 'resourceId' | 'completedAt'>[]
		| null
	resourceIdToMarkComplete?: string | null
}) {
	const requiredResources = flattenNavigationResources(navigation).filter(
	...
	const completedResourceIds = new Set(
		(completedLessons || [])
			.filter(
				(
					progress,
				): progress is Pick<ResourceProgress, 'resourceId' | 'completedAt'> & {
					resourceId: string
				} => Boolean(progress.resourceId && progress.completedAt),
			)
			.map((progress) => progress.resourceId),
	)

	if (resourceIdToMarkComplete) {
		completedResourceIds.add(resourceIdToMarkComplete)
	}

	const nextIncompleteResource =
		requiredResources.find(
			(resource) => !completedResourceIds.has(resource.id),
		) || null

	return {
		isModuleComplete: nextIncompleteResource === null,
		nextIncompleteResource,
		completedRequiredResourcesCount: requiredResources.filter((resource) =>
			completedResourceIds.has(resource.id),
		).length,
		totalRequiredResourcesCount: requiredResources.length,
	}
}

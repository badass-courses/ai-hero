...
export async function getTutorial(moduleSlugOrId: string) {
	...
	if (!parsedTutorial.success) {
		await log.error('tutorial.parse.error', {
			tutorialId: moduleSlugOrId,
			slug: moduleSlugOrId,
			error: formatZodError(parsedTutorial.error),
			source: 'getTutorial',
		})
		return null
	}
...
export const batchUpdateResourcePositions = async (
	...
	try {
		await log.info('tutorial.batch-update.start', {
		...
		await log.info('tutorial.batch-update.complete', {
			batchSize: updates.length,
			duration,
		})
		...
		await log.error('tutorial.batch-update.error', {
			batchSize: updates.length,

...
describe('content-navigation', () => {
	...
	describe('getModuleCompletionState', () => {
		it('returns the earliest incomplete lesson when a later lesson is completed', () => {
			const lesson1 = createResource({
				id: 'lesson-1',
				type: 'lesson',
				fields: { slug: 'lesson-1', title: 'Lesson 1' },
			...
			const completionState = getModuleCompletionState({
				navigation,
				completedLessons: [{ resourceId: 'lesson-2', completedAt: now }],
				resourceIdToMarkComplete: 'lesson-3',
			})

			expect(completionState.isModuleComplete).toBe(false)
			expect(completionState.nextIncompleteResource?.id).toBe('lesson-1')
			expect(completionState.completedRequiredResourcesCount).toBe(2)
			expect(completionState.totalRequiredResourcesCount).toBe(3)
		})

		it('treats the module as complete when the final required lesson is marked complete', () => {
			const lesson1 = createResource({
				id: 'lesson-1',
				type: 'lesson',
				fields: { slug: 'lesson-1', title: 'Lesson 1' },
			...
			const completionState = getModuleCompletionState({
				navigation,
				completedLessons: [{ resourceId: 'lesson-1', completedAt: now }],
				resourceIdToMarkComplete: 'lesson-2',
			})

			expect(completionState.isModuleComplete).toBe(true)
			expect(completionState.nextIncompleteResource).toBeNull()
		})

		it('ignores solutions when calculating module completion', () => {
			const lesson = createResource({
				id: 'lesson-1',
				type: 'lesson',
				fields: { slug: 'lesson-1', title: 'Lesson 1' },
			...
			const completionState = getModuleCompletionState({
				navigation,
				completedLessons: [],
				resourceIdToMarkComplete: 'solution-1',
			})

			expect(completionState.isModuleComplete).toBe(false)
			expect(completionState.nextIncompleteResource?.id).toBe('lesson-1')
			expect(completionState.totalRequiredResourcesCount).toBe(1)
		})
	})
})

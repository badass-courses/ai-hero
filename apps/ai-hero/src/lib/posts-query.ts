...
const _getCachedAllPosts = unstable_cache(
	async () => getAllPosts(),
	['posts-v3'],
...
export async function getCachedAllPosts() {
	const result = await _getCachedAllPosts()
...
export async function writeNewPostToDatabase(
	...
	try {
		void log.debug('post.create.validate', {
			postType: input.postType,
			createdById: input.createdById,
			hasVideoResourceId: Boolean(input.videoResourceId),
		})
		const validatedInput = NewPostInputSchema.parse(input)
		const { title, videoResourceId, postType, createdById } = validatedInput
		void log.debug('post.create.validated', {
			postType,
			createdById,
			hasVideoResourceId: Boolean(videoResourceId),
		})

		const postGuid = guid()
		const newPostId = `post_${postGuid}`
		void log.info('post.create.id.generated', {
			postId: newPostId,
			postType,
			createdById,
		})
		...
		if (videoResourceId) {
			void log.debug('post.create.video-resource.fetch', {
				postId: newPostId,
				videoResourceId,
			})
			videoResource =
				await courseBuilderAdapter.getVideoResource(videoResourceId)
			void log.debug('post.create.video-resource.fetched', {
				postId: newPostId,
		...
		try {
			...
			void log.info('post.create.core.start', {
				postId: newPostId,
				postType,
				createdById,
			})
			...
			void log.info('post.create.core.success', {
				postId: post.id,
				postType,
			})

			// Step 4: Handle contributions
			void log.debug('post.create.contributions.start', {
				postId: post.id,
				createdById,
			})
			...
			void log.debug('post.create.contributions.success', {
				postId: post.id,
				createdById,
			})

			// Step 6: Create version
			void log.debug('post.create.version.start', {
				postId: post.id,
			})
			await createNewPostVersion(post)
			void log.info('post.create.version.success', {
				postId: post.id,
			})

			return post
		} catch (error) {
			void log.error('post.create.error', {
				postId: newPostId,
				error: getErrorMessage(error),
			})
		...
		void log.error('post.create.validation.error', {
			postType: input.postType,
			createdById: input.createdById,
			error: getErrorMessage(error),
		})
...
async function createCorePost({
	...
	try {
		void log.debug('post.create.core.insert', {
			postId: newPostId,
			postType,
			createdById,
		})
		...
		void log.debug('post.create.core.inserted', {
			postId: newPostId,
		})
		...
		void log.debug('post.create.core.fetched', {
			postId: post?.id ?? newPostId,
		})

		if (!post) {
			void log.error('post.create.core.not-found', {
				postId: newPostId,
			})
		...
		const postParsed = PostSchema.safeParse(post)
		if (!postParsed.success) {
			void log.error('post.create.core.parse.error', {
				postId: newPostId,
				error: postParsed.error.message,
			})
		...
		void log.error('post.create.core.error', {
			postId: newPostId,
			error: getErrorMessage(error),
		})
...
export async function writePostUpdateToDatabase(input: {
	...
	if (!currentPost) {
		void log.error('post.update.not-found', {
			postId: postUpdate.id,
			action,
			updatedById,
		})
	...
	if (!postUpdate.fields.title) {
		void log.error('post.update.title.missing', {
			postId: postUpdate.id,
			action,
			updatedById,
		})
	...
	if (
		...
		postUpdate.fields.slug !== currentPost.fields.slug
	) {
		postSlug = postUpdate.fields.slug
	}

	void log.debug('post.update.metadata.calculate', {
	...
	void log.debug('post.update.metadata.calculated', {
		duration,
		timeToRead,
		videoResourceId,
	...
	try {
		void log.debug('post.update.db.write', {
			postId: currentPost.id,
			action,
		})
		...
		void log.error('post.update.db.error', {
			postId: currentPost.id,
			action,
			error: getErrorMessage(error),
		})
	...
	if (!updatedPost.success) {
		void log.error('post.update.validate.error', {
			postId: currentPost.id,
			action,
			error: updatedPost.error.message,
		})
	...
	if (newContentHash !== currentContentHash) {
		void log.info('post.update.version.changed', {
	...
	void log.debug('post.update.typesense.start', {
		postId: updatedPost.data.id,
		action,
	})
	...
	try {
		...
		void log.info('post.update.typesense.success', {
			postId: updatedPost.data.id,
			action,
		})
	} catch (error) {
		void log.warn('post.update.typesense.failed', {
			postId: updatedPost.data.id,
			action,
			error: getErrorMessage(error),
		})
...
const _getCachedPostOrList = unstable_cache(
	async (slugOrId: string) => {
		const result = await getPostOrList(slugOrId)
		return result ?? null
	},
	['posts-v3', 'lists-v3'],
...
export async function getPostOrList(slugOrId: string) {
	...
	if (!parsed.success) {
		await log.error('post.content.parse.error', {
			error: parsed.error.message,
			slugOrId,
			resourceType: postOrList.type,
		})
		return null
	}

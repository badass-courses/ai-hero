...
async function getChargeDetails(merchantChargeId: string) {
	...
	if (merchantCharge && merchantCharge.identifier) {
		...
		if (
			...
			merchantSession.identifier.startsWith('cs_')
		) {
			// Only retrieve if it's a real checkout session (cs_ prefix).
			// Invoice-originated purchases store an invoice ID (in_ prefix) here.

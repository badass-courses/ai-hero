...
export function EditProductForm({ product }: { product: Product }) {
	...
	const form = useForm<z.infer<typeof productSchema>>({
		...
		defaultValues: {
			...
			fields: {
				...
				availableAfterDays:
					product.fields.availableAfterDays ??
					(product.type === 'cohort-archive'
						? DEFAULT_ARCHIVE_AVAILABLE_AFTER_DAYS
						: undefined),
				accessDurationDays:
					product.fields.accessDurationDays ??
					(product.type === 'cohort-archive'
						? DEFAULT_ARCHIVE_ACCESS_DURATION_DAYS
						: undefined),
			},
		},
	})
	...
	const isCohortArchive = form.watch('type') === 'cohort-archive'
		...
		>
				...
				render={({ field }) => {
					return (
						<FormItem className="px-5">
							...
							<FormDescription className="mt-2 text-sm">
								Select the product type used for checkout and access behavior.
							</FormDescription>
							<Select
								onValueChange={(value) => {
									field.onChange(value)
									if (value === 'cohort-archive') {
										...
										form.setValue('fields.availableAfterDays', undefined)
										form.setValue('fields.accessDurationDays', undefined)
									}
								}}
							...
							>
								...
								<SelectContent>
									...
									<SelectItem value="cohort-archive">Cohort Archive</SelectItem>
								</SelectContent>
							</Select>
							<FormMessage />
						</FormItem>
			...
			{isCohortArchive && (
				<>
					<FormField
						control={form.control}
						name="fields.availableAfterDays"
						render={({ field }) => {
							return (
								<FormItem className="px-5">
									...
									<FormDescription className="mt-2 text-sm">
										How many days after a cohort ends before it becomes part of
										the archive.
									</FormDescription>
									<FormControl>
										<Input
					...
					<FormField
						control={form.control}
						name="fields.accessDurationDays"
						render={({ field }) => {
							return (
								<FormItem className="px-5">
									<FormLabel className="text-lg font-bold">
									...
									<FormDescription className="mt-2 text-sm">
										How long archive access lasts after purchase.
									</FormDescription>
									<FormControl>
										<Input
											type="number"

...
export async function GET(request: Request) {
	try {
		...
		// Post with picked artwork from the Slack pipeline:
		// dark band on top with the title, artwork strip across the bottom,
		// Matt's portrait overlapping the right side of both. Falls through
		// to the default render below if coverImage isn't set, so all other
		// posts behave exactly as before.
		if (resource?.type === 'post' && resource?.fields?.coverImage?.url) {
			const artworkUrl = resource.fields.coverImage.url as string
			const mattUrl =
				'https://res.cloudinary.com/total-typescript/image/upload/v1777888071/matt-for-og.png'
			const STRIP_HEIGHT = 240
			const TOP_HEIGHT = 630 - STRIP_HEIGHT // 337
			...
			return new ImageResponse(
				...
				>
					...
					{/*
					 * Matt's portrait. Source is 349×374 (≈0.93 aspect). Scaled to
					 * height 540 keeps the natural ratio and lands a 504-wide
					 * portrait. Satori requires explicit pixel width — `auto`
					 * resolves to 0 and the image silently vanishes.
					 */}
					<img
						src={mattUrl}
						width={504}

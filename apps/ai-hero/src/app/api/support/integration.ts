...
export const integration: SupportIntegration = {
	...
	async searchContent(
		...
		if (
			...
			await log.error('support.typesense.config-missing', {
				query,
				types,
				limit,
				error: 'Missing Typesense configuration for content search',
				hasApiKey: Boolean(process.env.TYPESENSE_WRITE_API_KEY),
				hasHost: Boolean(process.env.NEXT_PUBLIC_TYPESENSE_HOST),
			})
			return {
				results: [],
		...
		try {
			...
			await log.error('support.typesense.search-failed', {
				query,
				types,
				limit,
	...
	async getContentAccess(userId: string): Promise<ContentAccess> {
		...
		for (const purchase of activePurchases) {
			...
			const purchaseEntitlements = userEntitlements.filter((entitlement) => {
				return entitlement.sourceId === purchase.id
			})

			const archiveEntitlements = purchaseEntitlements.filter((entitlement) => {
			...
			const archiveSummary = summarizeArchiveEntitlements(archiveEntitlements)
			...
			const modules = isArchiveProductType(product.type)
				? archiveSummary.modules.map((module) => ({
						...module,
						title: `${module.title} (archive entitlement)`,
					}))
			...
			const relevantEntitlement = purchaseEntitlements.find((entitlement) => {
				return entitlement.expiresAt
			})
			const expiresAt = archiveSummary.expiresAt
				? new Date(archiveSummary.expiresAt)
				: relevantEntitlement?.expiresAt

			const isExpired = expiresAt ? new Date(expiresAt) < new Date() : false
			const productName = isArchiveProductType(product.type)
				? `${product.name} (archive purchase ${purchase.id})`
				: product.name

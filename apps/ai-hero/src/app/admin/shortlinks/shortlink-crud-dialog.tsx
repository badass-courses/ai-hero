	...
	metadataJson: z
		...
		.superRefine((value, context) => {
			...
			try {
				...
				const result = CreateShortlinkSchema.shape.metadata.safeParse(parsed)
				if (!result.success) {
					context.addIssue({
						code: z.ZodIssueCode.custom,
						message: 'Metadata must match the v1 shortlink metadata schema',
					})
				}
			} catch {
				context.addIssue({
...
export default function ShortlinkCrudDialog({
	...
	const form = useForm<FormSchemaType>({
		...
		defaultValues: {
			...
			metadataJson: shortlink?.metadata
				? JSON.stringify(shortlink.metadata, null, 2)
				: '',
		},
	})
	...
	const handleSubmit = async (values: FormSchemaType) => {
		const metadata = values.metadataJson?.trim()
			? CreateShortlinkSchema.shape.metadata.parse(
					JSON.parse(values.metadataJson),
				)
			: null
		const metadataJson = values.metadataJson || ''
		const existingMetadataJson = shortlink?.metadata
			? JSON.stringify(shortlink.metadata, null, 2)
			: ''
		const didMetadataChange = metadataJson !== existingMetadataJson
						...
						<FormField
							...
							render={({ field }) => (
								<FormItem>
									...
									<FormControl>
										<Textarea
											placeholder='{"schemaVersion":1,"campaign":"cohort_004","campaignPhase":"warmup","sourceSurface":"broadcast","sourceId":"cc004_warmup_01","createdFor":"campaign"}'
											className="min-h-32 font-mono text-xs"
											{...field}
										/>
									</FormControl>
									<FormDescription>
										Structured campaign metadata snapshotted onto click events.
									</FormDescription>
									<FormMessage />
								</FormItem>
							)}

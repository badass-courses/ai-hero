...
import {
  ...
  type AuthenticatedOAuthLinkSession,
} from "./oauth-link-session";
...
type SessionLookup = Parameters<
  typeof createAuthenticatedOAuthLinkSessionResolver
>[0]["getSessionAndUser"];
...
export function createEvergreenClaimHttp(options: {
  ...
  if (options.enabled && !options.secret)
    throw new Error("Claim CSRF secret required");
  const now = options.now ?? (() => new Date());
  const mac = (session: string, value: string) =>
  ...
  const valid = (session: string, token: unknown) => {
    ...
    return (
      ...
      timingSafeEqual(
        ...
        Buffer.from(mac(session, `${expiry}.${nonce}`), "hex"),
      )
    );
  };
  const reply = (status: ClaimStatus, code = 200, csrf?: string) =>
    Response.json(
      { status, ...(csrf ? { csrf } : {}) },
      {
        status: code,
  ...
  return async (request: Request): Promise<Response> => {
    ...
    try {
      ...
      for (const part of (request.headers.get("cookie") ?? "").split(";")) {
        ...
        if (!(authSessionCookieNames as readonly string[]).includes(name))
          continue;
        if (cookies.size || cookies.has(name)) return reply("unavailable", 403);
        cookies.set(name, decodeURIComponent(part.slice(index + 1)));
      }
      const session = await createAuthenticatedOAuthLinkSessionResolver({
        getCookieStore: () => ({
          delete: () => undefined,
          get: (name) =>
            cookies.has(name) ? { value: cookies.get(name)! } : undefined,
        }),
        getSessionAndUser: options.getSessionAndUser,
        now,
      })();
      if (!session) return reply("verification-needed", 401);
      if (request.method === "GET")
        return reply(
          await options.application.status(session),
          200,
          issue(session.sessionToken),
        );
      if (
      ...
      if (!reader) return reply("unavailable", 400);
      ...
      if (
        ...
        !("csrf" in body) ||
        !valid(session.sessionToken, body.csrf)
      )
        return reply("unavailable", 403);

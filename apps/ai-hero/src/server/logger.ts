 ...
 */
function isDirectAxiomIngestEnabled(): boolean {
	return process.env.AXIOM_DIRECT_INGEST === 'true'
}

function isAxiomConfigured(): boolean {
	return Boolean(
		isDirectAxiomIngestEnabled() &&
...
function logStructuredToConsole(level: LogLevel, entry: LogEntry): void {
	try {
		...
		logToConsole(level, {
			_time: entry._time,
			level,
			event: `${entry.event}.console_serialize_failed`,
		})
	}
}
...
async function write(
	...
	// Vercel already ships structured console logs to the vercel dataset.
	// Direct SDK ingest is opt-in because serverless background flushes can time out
	// and then pollute production logs with Axiom transport errors.
	if (env.NODE_ENV === 'development') {
		logToConsole(level, entry)
	} else if (level !== 'debug') {
		logStructuredToConsole(level, entry)
	}

	// If direct Axiom ingest is not configured, Vercel console ingestion is the sink.
	if (!isAxiomConfigured()) {
		return
	}
...
// Note: shutdown handlers (process.on/SIGINT/SIGTERM) are intentionally
// omitted here because this module is imported by Edge Runtime routes
// (e.g. /api/certificates) where process.on/process.exit are not available
// and Turbopack flags them as errors even behind runtime guards.
// Call log.flush() explicitly in long-running server contexts if needed.

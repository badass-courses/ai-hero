...
import { createRuntimeAuthCookiePolicyResolver } from '@/server/auth-cookie-policy'
...
export const getRuntimeAuthCookiePolicy =
	createRuntimeAuthCookiePolicyResolver({
		getHeaders: headers,
		getAuthUrl: () => process.env.AUTH_URL ?? process.env.NEXTAUTH_URL,
	})
...
const oauthContainmentSignInCallback = createOAuthContainmentSignInCallback({
	...
	isExplicitLinkAllowed: ({ provider, userId }) =>
		provider !== 'github' || isGithubOAuthLinkEnabledForUser(userId),
...
export async function getAuthOptions(request?: Request) {
	const cookiePolicy = await getRuntimeAuthCookiePolicy(request)
	return {
		...authOptions,

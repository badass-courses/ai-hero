...
export const createPPPCreditCouponsForPurchasers = inngest.createFunction(
	...
	async ({ event, step }) => {
		...
		const merchantCouponsByAmount: {
			merchantCoupons: Array<[number, string]>
			summary: { created: number; totalAmounts: number }
		} = await step.run('create merchant coupons for each amount', async () => {
		...
		const couponsByAmount: {
			coupons: Array<[number, string]>
			summary: { uniqueAmounts: number; totalAmounts: number }
		} = await step.run('create coupons for each unique amount', async () => {
			const coupons = new Map<number, string>()
			const merchantCouponsMap = new Map<number, string>(
		...
		const couponData: {
			couponData: CouponEntry[]
			summary: { totalPurchases: number; mappedPurchases: number }
		} = await step.run('prepare coupon data for entitlements', async () => {
			const couponsMap = new Map<number, string>(couponsByAmount.coupons)
			const data: Array<{
				couponId: string
				userId: string
				amountInCents: number

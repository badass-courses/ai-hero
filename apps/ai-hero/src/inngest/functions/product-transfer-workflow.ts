...
async function handleProductTransfer({
	...
	if (!archiveProduct && (!productType || !config)) {
	...
	if (!archiveProduct && resourceContexts.length === 0) {
	...
	const resourceDataMap = !archiveProduct
	...
	if (!isTeamPurchase && ['Valid', 'Restricted'].includes(purchase.status)) {
		...
		if (archiveProduct) {
			await transferArchivePurchaseEntitlements(step, {
				purchase,
				product,
				sourceUser,
				targetUser,
			...
			await step.run(`log archive transfer completed`, async () => {
				log.info('Archive purchase transfer completed successfully', {
					purchaseId: purchase.id,
					productId: product.id,
					sourceUserId: sourceUser.id,
					targetUserId: targetUser.id,

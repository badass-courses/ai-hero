...
/**
 * Homepage curation over the SAME three buckets (`ActivityLadder`).
 *
 * The homepage cannot afford `/learn`'s four-to-five full rows per goal, and
 * it is answering a different question: a first-time visitor deciding whether
 * anything here is for them. So each rung gets three picks and an explicit
 * `audience` line.
 *
 * Why an audience line and not tabs or an accordion: an experienced dev asked
 * to self-select will skip "How do I get started?" and never see the
 * fundamentals — which is exactly the material most of them are missing. Laid
 * out as three open rungs, the confident reader scrolling to their own row
 * still reads the titles above it. Self-location without self-exclusion.
 *
 * Picks deliberately avoid the "My latest posts" grid below, so no resource
 * appears twice on the page.
 */
export type ActivityRung = {
	/** Matching `GOAL_SECTIONS` id — the taxonomy is single-sourced there. */
	goalId: string
	/** Who this rung is for, in plain terms. Not a difficulty badge. */
	audience: string
	/** Exactly three slugs, curated for the homepage. */
	slugs: string[]
}

export const ACTIVITY_RUNGS: ActivityRung[] = [
	{
		goalId: 'get-started',
		...
		slugs: [
			'what-is-an-llm',
			'what-is-an-agent',
			'personal-software-is-insane-in-the-age-of-ai-u2hx2',
		],
	},
	{
		goalId: 'write-code-like-me',
		audience: 'If you can already code',
		slugs: [
			'a-complete-guide-to-agents-md',
			'plan-mode-introduction',
			'essential-ai-coding-feedback-loops-for-type-script-projects',
		],
	},
	{
		goalId: 'get-the-most-out',
		audience: 'If you use an agent every day',
		slugs: [
			'how-to-kill-the-bloat-in-claude-codes-system-prompt',

...
export const Share = ({
	...
	const shareText = `${title ? `${title} ` : ''}by ${config.twitter.handle}`
	const shareOptions = [
		{
			label: 'X',
			platform: 'x' as const,
		...
		},
		{
			...
			icon: <Linkedin className="size-6" aria-hidden="true" />,
			className: 'bg-secondary text-secondary-foreground',
		},
		{
			label: 'Email',
			platform: 'email' as const,
			href: `mailto:?subject=${encodeURIComponent(title ?? 'AI Hero')}&body=${encodeURIComponent(url)}`,
	...
	return (
        ...
        ${url}`)}`}
			...
			<button type="button" className="text-nowrap text-sm" onClick={copyUrl}>
				Copy URL
			</button>
		</div>
	)

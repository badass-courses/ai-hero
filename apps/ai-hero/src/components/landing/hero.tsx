...
export async function Hero({
	...
	return (
		...
		>
			{/* The animation is the hero's ground, not the right panel's: it spans
			    the whole header behind the copy and fades out on the VERTICAL, so
			    it plays in the open air above the headline and has gone quiet by
			    the time the reader is at the actions and the stats. */}
			{!video && (
				<div
					aria-hidden
			...
			{/* 88 / 72 top and bottom, vertically centred on desktop. On mobile the
			    ~200px of top padding is deliberate open stage: the animation gets
			    the top of the panel to itself before the copy starts. */}
			<div className="relative z-10 flex flex-col justify-center px-[18px] pb-14 pt-[200px] sm:px-11 sm:pb-[72px] sm:pt-[88px]">
				<div>
			...
			<div
				className={cn(
					...
					// Without a video the panel holds only the portrait, and the
					// portrait is desktop-only: on mobile the animation has the top
					// of the hero and the nav already carries Matt's face.
					video ? 'flex min-h-80' : 'hidden md:flex',
				)}
			>
				{video ? (
					...
					// it, standing over the shared ground the header now draws.
